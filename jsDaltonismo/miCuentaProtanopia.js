document.addEventListener('DOMContentLoaded', () => {
    ...
    // Volver al inicio (cerrar overlay) - Navega a index protanopia
    btnBackToHome?.addEventListener('click', () => {
        try {
            if (window !== window.parent) {
                window.parent.postMessage({ type: 'close-account-protanopia' }, '*');
            } else {
                // Navegar a la versión protanopia del index si existe
                window.location.href = '../indexProtanopia.html';
            }
        } catch (err) {
            window.location.href = '../indexProtanopia.html';
        }
